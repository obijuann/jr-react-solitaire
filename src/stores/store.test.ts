...
function makeDeck(): CardData[] {
  ...
  for (const s of suits) {
    for (const r of ranks) {
      deck.push({ rank: r as any, suit: s as any, face: 'down' });
    }
  }
  return deck;
}
...
describe('Zustand store actions', () => {
  ...
  it('checkGameState triggers game win when foundation full', () => {
    ...
    const makeCard = (r: string, s: string) => ({ rank: r as any, suit: s as any, face: 'up' } as CardData);
    const suits = ['clubs','diamonds','hearts','spades'];
    const ranks = ['ace','2','3','4','5','6','7','8','9','10','jack','queen','king'];
    const foundation = suits.map(s => ranks.map(r => makeCard(r, s)));
    useStore.setState({ playfield: { draw: [], waste: [], foundation: foundation as any, tableau: [[],[],[],[],[],[],[]] } });
